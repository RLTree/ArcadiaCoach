...
- Added a model picker (GPT-5, GPT-5 Mini, GPT-5 Codex) with capability-aware warnings so learners can choose their preferred backbone while staying within tool limits.
...
- `ChatPanel` hosts the new model picker and disables web/file UI affordances automatically when a model (e.g., GPT-5 Codex) lacks those tools.
...
2. **Manual QA:**
   ...
   - Switch between GPT-5 Mini and GPT-5 Codex; verify the UI disables web search/file uploads for Codex and that backend logs show the requested model/effort mapping.
